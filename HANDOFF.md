...
| `wordpress/catp-connect-acf.json` | The 9 post types and the 18 field groups in one ACF import. Custom Fields → Tools → Import, once. |
...
- **A post type** (rename, change what it supports, add one): do it in **Custom Fields → Post Types** in the WordPress admin. Since 0.14.0 the plugin registers none.
- **A field** (add/rename/reorder, change a dropdown, rewire a relation): do it in **Custom Fields** in the WordPress admin. No code, no re-upload, no re-zip. Since 0.12.0 the plugin does not register fields at all — ACF owns them, from the one-time import of `wordpress/catp-connect-acf.json`.
